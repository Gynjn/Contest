...
while True:
    ...
    detect, flag = object_detection(pred)
    if flag == 1:
        # avidance_processor = avoidance(ser, capture)
        avidance_processor = avoidance(capture)
        avidance_processor.action(is_outside(cam3_hsv), threshold)
