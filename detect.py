...
while True:
    ...
    # hsv =  cv2.cvtColor(cam2_bird, cv2.COLOR_BGR2HSV)
    # hsv = cv2.cvtColor(cam2_bird, cv2.COLOR_BGR2HSV)
    # hsvLower = np.array([0, 0, 200])
    # hsvUpper = np.array([180, 100, 255])
    # hsv_mask = cv2.inRange(hsv, hsvLower, hsvUpper)  # HSV에서 마스크를 작성
    # cam3_hsv = cv2.bitwise_and(cam2_bird, cam2_bird, mask=hsv_mask)
    ...
    # cam5_hough = cam4_roi
    gray = cvt_binary(cam3_hsv)
    # cam4_roi = roi_cutting(gray, 240)
    gray = roi_cutting(gray, 200)
    # cam4_roi = roi_cutting(gray, 0)
    # ob_img = preprocess(ob_cam, device='cpu')
    # pred = detect_net(ob_img)
    # pred = non_max_suppression(pred)[0]
    # detect, _ = object_detection(pred)
    # if detect[1] != None:
    #     traffic_box = detect[1]
    #     print(traffic_box[2])
    #     print(box_center(traffic_box)[0])
    # print(flag)
    # if detect[0] != None:
    #     cross_box = detect[0]
    #     if box_center(cross_box)[1] >= 370 and box_center(cross_box)[1] <= 380 and flag == 0:
    #         print("yes!")
    #         message = 'a' + str(60) + 's'+str(0)
    #         ser.write(message.encode())
    #         # ser.write(b'H')
    #         flag = stopping(capture)
    #     else:
    #         if detect[1] != None:
    ...
    edges = cv2.Canny(blurred, 110, 180)
    lines = cv2.HoughLines(edges, 1, np.pi / 180, 30)
    # blurred = cv2.GaussianBlur(cam5_hough, (3, 3), 0)
    # edges = cv2.Canny(blurred, 30, 100)
    ...
    # cv2.imshow("cam_test4", cam5_hough)
